...
> Install dependencies, configure env, and run the instructions recipe locally. This recipe is **zero-key**: Agora manages the OpenAI vendor; `OPENAI_API_KEY` is optional.
...
- No OpenAI key required — Agora provisions OpenAI access on your behalf
...
| `OPENAI_API_KEY`        |          | —                                    | Optional — Agora manages the OpenAI key; set only for BYO-key |
| `OPENAI_MODEL`          |          | `gpt-4o-mini`                        | OpenAI model name passed to the managed vendor                |
| `REPLY_STYLE`           |          | `normal`                             | `normal` (max_tokens 1024) or `short` (max_tokens 40, terse)  |
